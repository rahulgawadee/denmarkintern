...
export default function ContactSection({ locale, contact, briefTemplateHref }) {
  ...
  return (
    <section id="contact" className="bg-[#f5f5f5] border-y-2 border-[#d4d4d4] py-20 sm:py-24 relative overflow-hidden">
      {/* Subtle decorative elements */}
      <div className="absolute top-0 right-0 w-96 h-96 bg-[#2b2b2b]/5 rounded-full blur-3xl" />
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-[#525252]/5 rounded-full blur-3xl" />
      
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-12">
          <h2 className="text-3xl sm:text-4xl font-bold text-[#2b2b2b] mb-3">{contact.hero}</h2>
          <p className="text-lg text-[#737373] mb-6">{description}</p>
          <Button asChild className="bg-[#2b2b2b] hover:bg-[#525252] text-white shadow-md hover:shadow-lg transition-all hover:scale-105 font-bold">
        ...
        <Tabs defaultValue="quick" className="w-full">
          <TabsList className="grid w-full grid-cols-2 gap-2 mb-8 bg-white border-2 border-[#d4d4d4] p-2 h-auto">
            <TabsTrigger 
              value="quick" 
              className="data-[state=active]:bg-[#2b2b2b] data-[state=active]:text-white text-[#2b2b2b] font-semibold py-2.5 px-2 sm:px-4 rounded-md transition-all data-[state=active]:shadow-md flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 text-xs sm:text-sm"
            >
